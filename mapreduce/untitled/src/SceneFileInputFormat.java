...
public class SceneFileInputFormat extends FileInputFormat<RowPairWritable, Text>
{
    @Override
    ...
    @Override
    public RecordReader<RowPairWritable, Text> getRecordReader(
            InputSplit split, JobConf job, Reporter reporter)
            throws IOException
    {
        return new SceneRecordReader((SceneSplit) split, job);
    }
    ...
    public InputSplit[] getSplits(JobConf job, int useless) throws IOException
    {
        ...
        String content = new Scanner(new InputStreamReader(fs.open(fileName))).useDelimiter("\\Z").next();

        SceneSplit[] result = new SceneSplit[2];
        result[0] = new SceneSplit(0, 200, new Text(content));
        result[1] = new SceneSplit(200, 399, new Text(content));
